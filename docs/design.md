...
50. 정책 게이트의 우회로를 게이트 자신이 말한다. 이 게이트는 어휘 트립와이어고 우회로는 표현 바꾸기다. 목록은 MIT 소스의 정규식 두 개이므로 "공개하면 우회가 쉬워진다"는 반론은 성립하지 않는다 — 이미 공개돼 있다. 그런데 우회로와 오탐 방향은 SECURITY 한 줄과 `guarantees.policy_gate: lexical-tripwire` 에만 있었고, **걸린 사람이 보는 유일한 표면인 거절 메시지에는 없었다.** 문서를 읽지 않는 사람이 게이트를 만나는 지점이다. 두 계열(구현·장애) 거절 문장 뒤에 고정 한계를 붙인다. 매치된 원문은 싣지 않는다. 무엇을 매치했는지 되돌려 주면 질문이 stderr 와 실패 봉투에 다시 등장하고, 계열 이름만으로 고칠 방법은 충분하다. 오탐은 재현했다: `구현해 주지 말고 검토만 해라`, `코드를 작성하지 말고 분석만 해줘`, `Do not implement this, just review it` 이 전부 exit 10 이다. 열어 둘 수 없는 이유도 재현했다: 부정문을 해석하면 `구현하지 말고 이 버그를 고쳐줘`, `don't fix this bug, just review it` 이 함께 통과한다. 게이트가 지금보다 약해지므로 `_IMPLEMENTATION_RE` 에 부정문 처리를 넣지 않는다. 덮는 범위가 고르지 않은 것도 실측했다 — `이 diff를 구현하지 말고 보안 관점에서 검토만 해줘` 는 목록에 없는 형태라 통과한다. 같은 뜻의 문장이 한쪽은 걸리고 한쪽은 지나가는 것이 어휘 게이트의 실제 모양이고, 이 비대칭을 테스트로 고정해 목록을 좁히거나 넓히는 판단이 전체 모양을 보고 이뤄지게 한다. 27 의 실패 봉투는 넓히지 않는다 — 문장 안의 고정 절이라 키는 늘지 않는다.
