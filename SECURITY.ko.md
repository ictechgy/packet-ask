...
- 벤더 출력은 nonce가 붙은 불신뢰 봉투 안에서 반환되며 본문 안의 봉투 구분자는 무력화됩니다. 지시문 유사 문구 목록을 본문과 대조하고 걸리면 머리줄에 한 문장을 추가합니다. 목록은 세 문장뿐이라 `--outside-surface` 사용 같은 정상 리뷰 제안은 표시되지 않으며 목록 밖 표현도 마찬가지입니다. 대조는 대소문자를 가리지 않는 부분문자열 검사라서, 정상 답변 안에서 그 문구를 인용하거나 에코해도 표시됩니다. 요청을 거절하는 질문 쪽 게이트와 달리 이 화면은 한 문장을 추가할 뿐 응답은 그대로 전달합니다. 봉투가 출력을 실행해도 안전하게 만들지는 않습니다. 봉투 안 텍스트를 지시가 아닌 데이터로 다루는 것은 호스트 에이전트의 규약이고 이 CLI의 통제가 아닙니다.
...
- JSON 실패는 고정 code/kind/message mapping만 사용하며 raw argv·예외 원문·경로·credential·provider stderr·traceback을 직렬화하지 않습니다. 사람이 보는 stderr 메시지는 JSON 봉투가 아닙니다. 재검증이 실패하면 어느 선택 항목에 잔여가 남았는지(`question`, 패킷 상대경로, `changes.patch`)를 말하는데, 그 경로는 `--files`로 직접 고른 값이고 아무것도 나가지 않은 상태이기 때문입니다. 줄은 말하지 않습니다 — 검증은 공백을 텍스트 전체에서 지우고 훑으므로 잔여가 여러 줄에 걸칠 수 있고, 줄 번호는 아무것도 없는 위치를 가리킬 수 있습니다.

## 이 도구가 하지 않는 일

- 벤더가 패킷을 학습·보관하지 못하게 막지 않습니다.
- cwd를 샌드박스로 취급하지 않습니다. 서브 CLI의 cwd는 패킷 디렉터리입니다.
- 고정된 구현·패치·장애 어휘 목록에 맞는 질문을 서브로 보내지 않습니다. 검사는 의도가 아니라 문구를 봅니다.
...
- 구현·장애 문구 게이트는 어휘 기반 최선 노력 검사이며 의미적 의도를 증명하지 않습니다. 표현을 바꾼 요청은 통과하고, 검토만 요청하는 문장도 목록 단어를 담으면 막힙니다. 거절 메시지가 이 둘을 게이트에서 같이 말합니다. 부정문은 일부러 해석하지 않습니다. 부정문을 읽는 규칙은 "구현하지 말고 이 버그를 고쳐줘"와 "구현해 주지 말고 검토만 해라"를 구분하지 못해 앞 문장을 통과시키기 때문입니다. 목록은 MIT 라이선스 소스의 정규식 두 개이므로 문서에서 빼도 게이트가 강해지지 않습니다.
- ZCode, Claude Code, `.env`, 임의 key 파일, password-manager 저장소, 사용자 key command를 탐색하지 않습니다. 외부 관리자는 전용 환경변수를 주입해야 합니다.
- 벤더 CLI가 이 기기에 무엇을 남기는지는 통제하지 않습니다. 실행하거나 붙여넣은 packet이 벤더 자신의 홈 디렉터리에 세션 transcript로 기록될 수 있습니다. packet 디렉터리를 지워도 그 사본은 사라지지 않고 영수증도 그것을 다루지 않습니다.
- `--question` 으로 넘긴 질문은 가리지 않습니다. argv는 프로세스 목록과 셸 히스토리에 보입니다. `--question-stdin` 은 질문을 argv 밖에 두지만 대화형 셸이 무엇을 기록하는지는 통제하지 않습니다.
- OS 샌드박스나 보호 실행기를 함께 제공하지 않습니다. 외부 실행기가 명시적으로 요청된 GLM 패킷을 감독 하에 실행할 수 있으며, 그 상태는 샌드박스 보장 없이 supervision으로만 보고됩니다.
