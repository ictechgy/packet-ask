...
- 벤더 출력은 nonce가 붙은 불신뢰 봉투 안에서 반환되며 본문 안의 봉투 구분자는 무력화됩니다. 지시문 유사 문구 목록을 본문과 대조하고 걸리면 머리줄에 한 문장을 추가합니다. 목록은 세 문장뿐이라 `--outside-surface` 사용 같은 정상 리뷰 제안은 표시되지 않으며 목록 밖 표현도 마찬가지입니다. 봉투가 출력을 실행해도 안전하게 만들지는 않습니다. 봉투 안 텍스트를 지시가 아닌 데이터로 다루는 것은 호스트 에이전트의 규약이고 이 CLI의 통제가 아닙니다.
