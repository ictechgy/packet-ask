...
- 구현·장애 문구 게이트는 어휘 기반 최선 노력 검사이며 의미적 의도를 증명하지 않습니다. 표현을 바꾼 요청은 통과하고, 목록 단어를 우연히 담은 검토 전용 문장은 막힙니다. 거절 메시지가 이 둘을 게이트에서 같이 말합니다. 부정문은 일부러 해석하지 않습니다. 해석하면 "구현하지 말고 이 버그를 고쳐줘"도 함께 통과하기 때문입니다. 목록은 MIT 라이선스 소스의 정규식 두 개이므로 문서에서 빼도 게이트가 강해지지 않습니다.
