...
- It finds `claude` / `kimi` executables in allowlist directories, resolves symlinks to a canonical target, and requires the entry directory, target directory, and regular executable to be root/current-user owned and not group- or world-writable. This narrows path replacement but is not fd-based execution. **Publish origin and signatures are not verified.**
