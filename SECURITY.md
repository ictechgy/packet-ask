...
- Known token families are checked symmetrically in primary scrub and shadow verification. Secret literals, URL userinfo, and PEM headers also use the shadow. Canonical dotted Korean mobile numbers are scrubbed, including one behind a leading dot such as `v1.010.1234.5678`; mixed dot/dash/space forms fail closed. A non-canonical dotted run that only normalizes to a mobile shape is rejected instead of sent, and a number followed by another dot+digit group is not treated as a phone. This does not claim general E.164 coverage.
...
- It finds `claude` / `kimi` / `git` executables in allowlist directories, resolves symlinks to a canonical target, and requires the immediate entry directory, immediate target directory, and regular executable to be root/current-user owned and not group- or world-writable by mode bits. This narrows path replacement but does not inspect ACLs and is not fd-based execution. A `0775` user-private-group `~/.local/bin` is rejected deliberately; make the directory `0755` or use another private allowlist directory. Doctor reports a fixed owner/mode failure without the path. When `--help` cannot be read, doctor reports existence only, without these checks. **Publish origin and signatures are not verified.**
...
- Vendor output is returned inside a nonce-marked untrusted envelope, and envelope markers inside the body are neutralized. An instruction-like phrase list is matched against the body; when it hits, one header sentence is added. The list is three phrases, so an ordinary review suggestion such as using `--outside-surface` is not flagged, and phrasings outside the list are not either. The match is a case-insensitive substring test, so a phrase quoted or echoed inside a legitimate answer also triggers it. Unlike the question gate, which rejects the request, this screen only adds a sentence and passes the response through unchanged. The envelope does not make output safe to execute. Treating envelope text as data rather than instructions is the host agent's rule, not a control of this CLI.
...
- JSON failures use fixed code/kind/message mappings. They never serialize raw argv, exception text, paths, credentials, provider stderr, or tracebacks. The human stderr message is not the JSON envelope. When one item fails re-verification it names that item (`question`, a packet-relative path, or `changes.patch`) — a path you selected yourself, with line, control, and bidi characters escaped, and nothing was sent. When every item passed and only the assembled packet failed, it says so and names no item, because the trigger is then in the framing or in how items join. It never names a line: verification strips whitespace across the whole text, so a residue can span lines and a line number could point at nothing.
...
- It does not send a question that matches the fixed implementation/patch/incident word list to a sub. The check is on wording, not on intent.
...
- The implementation/incident wording gate is lexical and best-effort; it does not prove semantic intent. A rephrased request passes it, and a question that only asks for a review is also blocked when it contains one of the listed words; the rejection message says both at the gate. Negations are deliberately not interpreted, because a rule that reads them cannot separate "do not implement this, fix the bug" from "do not implement this, just review it" and would let the first through. The list is two regular expressions in MIT-licensed source, so keeping it out of the docs would not make the gate stronger.
...
| `PACKET_ASK_LEDGER` | Absolute path to an opt-in append-only ledger. A run appends an `egress` line before the vendor starts and a `result` line after the response settles; a killed run leaves only the first. Never holds the question, file bodies, the vendor's answer, or vendor stderr. `packet-ask ledger summary` reads it back and prints counters only — no paths, no question, no bodies. Reading checks the absolute path, rejects symlinks, requires a regular file owned by the current user, and stops at a size cap; it does not enforce the file mode and does not re-run the worktree check, because the summary carries no file content into any packet |
