...
- The implementation/incident wording gate is lexical and best-effort; it does not prove semantic intent. A rephrased request passes it, and review-only wording that happens to contain a listed word is blocked; the rejection message says both at the gate. Negations are deliberately not interpreted, because reading them would also let "do not implement this, fix the bug" through. The list is two regular expressions in MIT-licensed source, so keeping it out of the docs would not make the gate stronger.
