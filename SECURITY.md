...
- JSON failures use fixed code/kind/message mappings. They never serialize raw argv, exception text, paths, credentials, provider stderr, or tracebacks. The human stderr message is not the JSON envelope: when re-verification fails it names which selected item kept the residue (`question`, a packet-relative path, or `changes.patch`), because that path is one you passed to `--files` and nothing was sent. It never names a line — verification strips whitespace across the whole text, so a residue can span lines and a line number could point at nothing.
