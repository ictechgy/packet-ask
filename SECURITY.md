...
- Known token families are checked symmetrically in primary scrub and shadow verification. Secret literals, URL userinfo, and PEM headers also use the shadow. Canonical dotted Korean mobile numbers are scrubbed, including one behind a leading dot such as `v1.010.1234.5678`; mixed dot/dash/space forms fail closed. A non-canonical dotted run that only normalizes to a mobile shape is rejected instead of sent, and a number followed by another dotted group is not treated as a phone. This does not claim general E.164 coverage.
