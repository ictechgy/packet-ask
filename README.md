...
a proof of intent. A rephrased request passes it and review-only wording that
contains a listed word is blocked; the rejection message says both. Negations
are deliberately not interpreted. Launch adapters disable vendor tools and use
the packet as the child cwd, but this is not OS-level filesystem confinement.
