...
def _lexical_rejection(key: str) -> PolicyError:
    ...
    (표현을 바꾸면 통과)와 오탐 방향(검토만 요청하는 문장도 목록 단어를
    담으면 막힘)을 거절 지점에서 같이 말한다.

    질문 원문과 매치 위치는 회신하지 않는다. 되돌려 주면 질문이 stderr 와
    실패 봉투에 다시 등장하고, 계열 이름만으로 고칠 방법은 충분하다.
    단, 계열 문장 **자체가** 목록 단어를 담는다(운영 사고 계열의
    "Production incident"). 그래서 주장하는 것은 "목록 단어가 메시지에
    없다"가 아니라 "사용자 질문을 싣지 않는다"다.
    """
