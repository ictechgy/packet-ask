...
- `packet-ask doctor` ends with a fixed `signals=verification:flags-mentioned,sandbox:unproven,signatures:unverified` line. A provider row that says `launch` means help text mentioned the flags, not that anything is sandboxed or signed.
