...
- `PACKET_ASK_LEDGER` (absolute path, opt-in) appends two JSON lines per run: an `egress` line before the vendor starts (scope, bytes, digest, redaction counts) and a `result` line after the response settles (outcome, output bytes, whether the instruction-like tripwire fired). No question, no file bodies, no vendor answer. If the egress line cannot be written the vendor does not run; if the result line cannot be written you get a fixed warning and the answer is still printed. Do not set it to a path inside the repo.
