...
- `--effort {low,medium,high,xhigh,max}` selects vendor reasoning depth on `glm` and `claude`. Measured on one packet: low 108 s, medium 214 s, high 444 s, max 751 s — roughly 2x per step. Omit it for the vendor default (measured between high and max). Use `low` for a quick check and `max` when the answer is worth a 15-minute wait. The auto timeout rises with it (1200/1800/2700 s), so you rarely need `--timeout`. `paste` and `kimi` reject the flag.
