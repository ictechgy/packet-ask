...
- `--effort {low,medium,high,xhigh,max}` selects vendor reasoning depth on `glm` and `claude`. Measured on one packet: low 108 s, medium 214 s, high 444 s, max 751 s — about 2x per step up to `high`, then 1.7x. Omit it for the vendor default, which was not measured under control. Use `low` for a quick check and `max` when the answer is worth a 15-minute wait. The auto timeout rises with it (1200/1800/2700 s), so you rarely need `--timeout`. `paste` and `kimi` reject it. `PACKET_ASK_EFFORT` sets a default; `--effort` wins and `effort_source` records which applied.
...
- `PACKET_ASK_LEDGER` (absolute path, opt-in) appends two JSON lines per run: an `egress` line before the vendor starts (scope, bytes, digest, redaction counts) and a `result` line after the response settles (outcome, output bytes, whether the instruction-like tripwire fired). A killed run leaves only the egress line, and a line without `phase` is an egress line from an older version. No question, no file bodies, no vendor answer. If the egress line cannot be written the vendor does not run; if the result line cannot be written you get a fixed warning and the answer is still printed. Do not set it to a path inside the repo.
