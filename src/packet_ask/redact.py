...
# 숫자 경계가 없으면 긴 숫자열 한가운데를 전화번호로 읽는다. 바로 아래 dotted
# 후보 패턴은 이미 경계를 갖고 있다. 같지는 않다. 그쪽은 `(?<![\d.])` 로 점까지
# 배제해 더 좁다. 여기서 점을 배제하지 않는 것은 compact 가 점을 지우지 않아
# `1.01012345678` 같은 형태를 계속 잡기 위해서다. 방향이 fail-closed 라 안전하다.
# 좁아지는 것: 숫자에 바로 붙은 번호는 더 이상 잡지 않는다. `99901012345678`
# 같은 형태다. 이것은 수용한 절충이고 아래 테스트로 고정한다. scrub 의
# `_PHONE_RE` 도 같은 `(?<!\d)`/`(?!\d)` 경계를 쓰므로 두 단계가 같은 것을
# 전화번호로 본다는 점에서 오히려 정합적이다.
