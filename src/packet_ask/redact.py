...
# 괄호는 흔한 한국 표기다. 검증은 괄호를 지우고 훑으므로 스크럽이 이 형식을
# 못 지우면 검증만 잡아서 그 패킷은 영영 못 보낸다. 지울 수 있으면 보낼 수 있다.
# 짝으로만 허용하는 것은 의도 표현이다. `\(?...\)?` 로 느슨하게 써도 실제
# 동작은 같았다. 뒤쪽 대안이 여는 괄호 다음부터 매치를 시작할 수 있어
# `\(?` 가 더하는 것이 없기 때문이다. 측정해서 확인했다.
_PHONE_RE = re.compile(
    r"(?<!\d)(?:\+82[-\s]?)?"
    r"(?:\(0?1[016789]\)|0?1[016789])"
    r"[-\s]?\d{3,4}[-\s]?\d{4}(?!\d)"
)
# 왼쪽 경계는 숫자 바로 붙음만 자르고 점은 허용한다. 점을 같이 배제하면
# `v1.010.1234.5678` 처럼 점 접두가 붙은 canonical 번호가 scrub·verify 를
# 모두 통과해서 그대로 나갔다. 오른쪽 `(?!\d|\.\d)` 는 유지한다. 번호 뒤에
# 점+숫자가 더 있으면 전화번호가 아니라 더 긴 dotted 식별자(OID·버전열)의
# 일부로 보고 보존하는데, 그쪽까지 넓히면 식별자를 조각낸다. 보존은
# 점+**숫자**일 때만이다. `010.1234.5678.beta` 는 점 뒤가 숫자가 아니므로
# 번호로 보고 지운다. 실측한 잔여: `010.1234.5678.9012` 와 `010.1234.5678.9`
# 는 두 단계 모두 통과한다. 수용한 절충이고 아래
# `test_widened_dot_prefix_keeps_longer_dotted_run` 이 고정한다.
...
# 숫자 경계가 없으면 긴 숫자열 한가운데를 전화번호로 읽는다. 여기서 점을
# 배제하지 않는 것은 compact 가 점을 지우지 않아 `1.01012345678` 같은
# 형태를 계속 잡기 위해서다. 방향이 fail-closed 라 안전하다.
# 좁아지는 것: 숫자에 바로 붙은 번호는 더 이상 잡지 않는다. `99901012345678`
# 같은 형태다. 이것은 수용한 절충이고 아래 테스트로 고정한다. scrub 의
# `_PHONE_RE` 와 `_DOTTED_PHONE_RE` 도 같은 `(?<!\d)` 왼쪽 경계를 쓴다.
# dotted 후보의 왼쪽 경계를 scrub 보다 넓히지 않는 이유가 있다. verify 만
# 잡으면 scrub 이 못 지우므로 그 패킷은 영영 못 나간다.
# 같은 것은 **왼쪽 경계뿐**이다. 후보 모양은 scrub 보다 의도적으로 넓어서 점
# 위치를 묻지 않는다. compact 가 하이픈·공백·괄호를 지우므로 점 위치는 원문과
