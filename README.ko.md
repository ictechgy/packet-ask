...
`PACKET_ASK_LEDGER` 에 절대 경로를 지정하면 모든 task 실행이 JSON 두 줄을
...
않습니다. `not-observable` 은 `paste` 입니다. 답이 이 도구를 지나지 않으므로
되돌려 주는 패킷 본문을 출력으로 기록하지 않습니다. `output_hint` 는 지시문
유사 트립와이어가 발화했다는 사실만 남기고 문구는 남기지 않습니다. 0.11.0
이전이 쓴 줄에는 `phase` 가 없으며 전부 egress 줄입니다.
