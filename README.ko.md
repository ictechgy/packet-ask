...
`doctor: help-text-only`, `policy_gate: lexical-tripwire`,
`output_screen: lexical-tripwire`)가 실리고, 사람이 읽는 receipt 한 줄 끝에
...
네 키는 기전이 존재한다고 주장하며(`redaction`, `doctor`, `policy_gate`,
`output_screen`) 실제 동작에 테스트로 묶여 있습니다. 기전보다 오래 살아남은
상수는 기계 판독 가능한 거짓이기 때문입니다. 사람이 읽는 한 줄은 기전 서술 키
셋(`doctor`, `policy_gate`, `output_screen`)을 뺀 나머지를 모두 싣고 같은
상수에서 조립하므로 낡은 값을 인용할 수 없습니다. `redaction` 은 일부러 양쪽에
속합니다. 값 `denylist` 자체가 한계 서술이기 때문입니다. `inspect` 는 벤더를
띄우지 않는데도 벤더 키를 그대로 싣습니다. 이 객체는 한 번 실행의 측정값이
아니라 도구의 한계 목록이므로, 그 표면의 런치 상태와 같이 읽어야 합니다.

**이 목록은 전부가 아닙니다.** 가장 자주 오독되는 한계를 적은 것이지 모든 위험을
적은 것이 아닙니다. 실패 봉투는 그대로 고정 code/kind/message 만 담습니다.

사람이 보는 stderr 메시지는 그 봉투가 아닙니다. 재검증이 실패하면 어느 선택
항목에 잔여가 남았는지를 말합니다 — `question`, 패킷 상대경로, 또는
`changes.patch`. 영영 못 나가는 패킷이 무엇을 고쳐야 하는지 알려 줍니다. 각
항목은 단독으로 통과했는데 조립된 패킷만 실패하면 그 사실을 따로 말하고
프레이밍을 가리킵니다. `010.1234.5678.py` 같은 파일 **이름**은 항목 단위
검증이 보지 못하는 헤더를 통해 `packet.md` 에 실리므로, 본문을 grep 하면
아무것도 나오지 않습니다. 줄 번호는 보고하지 않습니다. 검증은 공백을 텍스트
전체에서 지우고 훑으므로 잔여가 여러 줄에 걸칠 수 있고, 줄 번호는 문제없는
줄을 가리킬 수 있습니다.
...
`PACKET_ASK_LEDGER` 에 절대 경로를 지정하면 task 실행이 JSON 두 줄을
덧붙입니다. 벤더 시작 **전**의 `egress` 줄과, 응답이 확정된 뒤의 `result` 줄입니다.
egress 줄은 egress 지점까지 도달한 실행을 기록하는 것이지 전달을 확인한 기록이
아닙니다. 이후 벤더가 실패해도 항목은 남으며, 감사 표면으로서는 그쪽이 안전한
방향입니다. 시각, phase, mode, provider, selector, 상대 경로, byte, packet digest,
redaction count, 결정된 timeout 이 들어갑니다.

result 줄은 packet digest 로 egress 줄과 짝짓고 시각, phase, mode, provider,
digest, `outcome`, `output_bytes`, `output_hint`, 그리고 실패했을 때만
`failure_code` 를 담습니다. `answered` 는 벤더가 출력 가드를 통과한 응답을
돌려줬다는 뜻이지 프로세스가 0 으로 끝난다는 뜻이 아닙니다 — cleanup 경고는
그 뒤에도 올 수 있습니다. `failed` 는 종료 코드만 싣고 벤더 stderr 는 싣지
않습니다. `not-observable` 은 벤더를 띄우지 않는 provider mode 입니다 — 지금은
`paste` 이고 `--dry-run` 도 그것을 강제합니다. 답이 이 도구를 지나지 않으므로
되돌려 주는 패킷 본문을 출력으로 기록하지 않습니다. `output_bytes` 는 정규화된
응답 본문의 byte 이며 봉투 프레임은 빼고 셉니다. `output_hint` 는 지시문
유사 트립와이어가 발화했다는 사실만 남기고 문구는 남기지 않습니다.

`phase` 가 없는 줄은 egress 로 읽고(0.11.0 이전은 쓰지 않았습니다), 모르는
phase 는 건너뛰세요. result 줄에는 `selector` 와 `paths` 가 없습니다. 짝은
digest 와 시각 순서로 짓습니다. 같은 패킷을 **동시에** 두 프로세스로 보내면
줄이 interleaving 할 수 있고, 그때는 파일만으로 어떤 result 가 어떤 egress 의
짝인지 정할 수 없습니다. result 줄이 없는 egress 줄은 응답이 확정되기 전에
끝난 실행입니다 — 죽임을 당했거나, 처리된 오류 경로 밖에서 죽었거나, result
기록에 실패했습니다. 마지막 경우는 stderr 에 고정 경고도 남으므로 그것만이
앞의 둘과 구별되는 단서입니다.

질문·파일 본문·벤더 답변·벤더 stderr 는 어느 줄에도 기록하지 않습니다.

`packet-ask ledger summary` 는 그 파일을 다시 읽어 카운터만 출력합니다: 줄 수
(빈 줄 제외), egress, result, outcome 세 개, `unpaired_egress`, `hint_hits`,
`skipped`, 두 byte 합계, 첫·마지막 시각. provider id 는 `--json` 에만 있고 사람
줄에는 없습니다 — 공백을 담은 사용자 별명이 토큰 분리를 깨지 않게 하려고
의도적으로 뺐습니다. 어느 출력도 경로·질문·본문을 싣지 않습니다. egress 줄에는
상대경로가 들어 있어도 그렇습니다. `--json` 은 같은 수치를 `packet-ask.v1`
객체로 돌려줍니다. 이 명령은 `PACKET_ASK_LEDGER` 를 읽고 그것이 없으면 exit 2
이며, 파일을 만들지는 않습니다. 읽기는 절대경로를 확인하고 심링크를 거절하고
현재 사용자 소유의 일반 파일만 읽으며, 접두어만 요약하는 대신 8 MiB 상한에서
멈춥니다. 파일 모드는 강제하지 않고 워크트리 검사도 다시 하지 않습니다 —
요약은 어떤 파일 내용도 패킷으로 옮기지 않기 때문입니다. 해석하지 못하는 줄,
이 버전이 받지 않는 phase·outcome·timestamp 형태·byte 값을 가진 줄은 요약을
중단하거나 억지로 세지 않고 `skipped` 로 셉니다. packet digest 가 없는 egress
줄은 짝짓지 않으므로 항상 `unpaired_egress` 에 들어갑니다.
...
`0600` 으로 강제합니다. **egress 줄을 기록하지
못하면 벤더를 실행하지 않고** 13 으로 끝냅니다. 조용히 빠뜨리는 대장은 없느니만
못합니다. result 줄은 대칭이 아닙니다. 그 시점에는 패킷이 이미 나갔고 답도 손에
있으므로, 기록에 실패하면 stderr 에 고정 경고만 남기고 실행은 성공으로 끝납니다.
opt-in 기록 때문에 답을 버리는 쪽이 더 큰 손실입니다. 변수를 비워 두면 기능은
꺼져 있습니다.
...
표현을 바꾼 요청은 통과하고, 검토만 요청하는 문장도 목록 단어를 담으면
막히며, 거절 메시지가 이 둘을 같이 말합니다. 부정문은 일부러 해석하지
않습니다. 런치 어댑터는 벤더 도구를 끄고 패킷을 자식 cwd로 사용하지만, 이는
OS 수준 파일시스템 격리가 아닙니다.
...
`ledger summary` 는 이 코드를 읽기에 재사용합니다. 여기서 `2` 는
`PACKET_ASK_LEDGER` 가 비어 있다는 뜻이고, `13` 은 대장이 읽기 검사를 통과하지
못했다는 뜻(심링크, 일반 파일 아님, 현재 사용자 소유 아님)이며, `14` 는 요약
상한보다 크다는 뜻입니다. 벤더 프로세스는 관여하지 않습니다.
