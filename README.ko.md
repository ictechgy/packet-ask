...
기전이 존재한다고 주장하는 네 키(`redaction`, `doctor`, `policy_gate`,
`output_screen`)는 실제 동작에 테스트로 묶여 있습니다. 기전보다 오래 살아남은
상수는 기계 판독 가능한 거짓이기 때문입니다. 사람이 읽는 한 줄은 한계 키만 싣고
같은 상수에서 조립하므로 낡은 값을 인용할 수 없습니다. `inspect` 는 벤더를 띄우지
않는데도 벤더 키를 그대로 싣습니다. 이 객체는 한 번 실행의 측정값이 아니라
도구의 한계 목록이므로, 그 표면의 런치 상태와 같이 읽어야 합니다.
